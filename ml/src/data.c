                    ...
                    , float hue, float saturation, float exposure, int center)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        image im = load_image_color(paths[i], size, size);

        /*
        char buf[80];
        sprintf(buf, "./train_%d_%d_%d.png", i, im.w, im.h);
        save_image(im, buf);
        */
#if 0
        image crop;
...
matrix load_image_simple(char *path, int w, int h)
{
    matrix X;
    X.rows = 1;
    X.vals = calloc(X.rows, sizeof(float*));

    image im = load_image_color(path, w, h);
...
void fill_truth_iseg(char *path, int num_boxes, float *truth, int classes, int w, int h, augment_args aug, int flip, int mw, int mh)
{
    ...
    while((fscanf(file, "%d %s", &id, buff) == 2) && i < num_boxes){
        ...
        if(b.w > 0){
            ...
            image mask = resize_image_color(crop, mw, mh);
...
void fill_truth_detection(char *path, int num_boxes, float *truth, int classes, int flip, float dx, float dy, float sx, float sy)
{
    ...
    printf("fill_truth_detection(): path=%s\n", path);
    ...
    printf("fill_truth_detection(): labelpath=%s\n", labelpath);

    int count = 0;
    box_label *boxes = read_boxes(labelpath, &count);
    printf("count.n=%d, id=%d, x=%f, y=%f, w=%f, h=%f\n"
        , count, boxes[0].id, boxes[0].x, boxes[0].y, boxes[0].w, boxes[0].h);
    ...
    for (i = 0; i < count; ++i) {
        ...
        h =  boxes[i].h;
        id = boxes[i].id;
        printf("count.i=%d, id=%d, x=%f, y=%f, w=%f, h=%f\n", i, id, x, y, w, h);
...
///Append on 2018-06-13
///load labels for simple
///Jung-JaeJoon(rgbi3307@nate.com)
matrix load_labels_path_simple(char *path, char **labels, int k, tree *hierarchy)
{
    matrix y = make_matrix(1, k);   ///k=classes

    if(labels){
        fill_truth(path, labels, k, y.vals[0]);
        if(hierarchy){
            fill_hierarchy(y.vals[0], k, hierarchy);
        }
    }
    return y;
}
...
data load_data_region(int n, char **paths, int m, int w, int h, int size, int classes, float jitter, float hue, float saturation, float exposure)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        image sized = resize_image_color(cropped, w, h);
        if(flip) flip_image(sized);
        random_distort_image(sized, hue, saturation, exposure);
        d.X.vals[i] = sized.data;
...
void *load_thread(void *ptr)
{
    ...
    if (a.type == OLD_CLASSIFICATION_DATA){
        ...
        *(a.resized) = resize_image_color(*(a.im), a.w, a.h);
...
void *load_threads(void *ptr)
{
    ...
    if (args.threads == 0) args.threads = 1;    ///32
    data *out = args.d;
    int total = args.n;     ///128
    free(ptr);
    data *buffers = calloc(args.threads, sizeof(data));
    pthread_t *threads = calloc(args.threads, sizeof(pthread_t));
    for(i = 0; i < args.threads; ++i){
        args.d = buffers + i;
        args.n = (i+1) * total/args.threads - i * total/args.threads;   ///4
...
data load_data_super(char **paths, int n, int m, int w, int h, int scale)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        int flip = rand()%2;
        if (flip) flip_image(crop);
        image resize = resize_image_color(crop, w, h);
...
data resize_data(data orig, int w, int h)
{
    ...
    for(i = 0; i < orig.X.rows; ++i){
        ...
        d.X.vals[i] = resize_image_color(im, w, h).data;
    }
    return d;
}

data load_data_augment(char **paths, int n, int m, char **labels, int k, tree *hierarchy, int min, int max, int size, float angle, float aspect
                    , float hue, float saturation, float exposure, int center)
{
    if(m) paths = get_random_paths(paths, n, m);    ///*random_paths
    data d = {0};
    d.shallow = 0;
    d.w=size;
    d.h=size;
...
data load_data_paths(char **paths, int n, int m, char **labels, int k, tree *hierarchy, int w, int h)
{
    if(m) paths = get_random_paths(paths, n, m);    ///*random_paths
    data d = {0};
    d.shallow = 0;
    d.w=w;
    d.h=h;
    d.X = load_image_paths(paths, n, d.w, d.h);
    d.y = load_labels_paths(paths, n, labels, k, hierarchy);
    if(m) free(paths);
    return d;
}

data load_data_simple(char *path, char **labels, int k, tree *hierarchy, int w, int h)
{
    data d = {0};
    d.shallow = 0;
    d.w=w;
    d.h=h;
    d.X = load_image_simple(path, d.w, d.h);
    ///d.y = load_labels_path_simple(path, labels, k, hierarchy);
    return d;
}
