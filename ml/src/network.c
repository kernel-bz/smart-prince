...
void forward_network(network *netp)
{
    ...
    network net = *netp;
    int i;
    pr_debug(">> FORWARD NETWORK[%d] ", net.n);
    for(i = 0; i < net.n; ++i){
        net.index = i;
        layer l = net.layers[i];
        if(l.delta){
...
void update_network(network *netp)
{
    ...
    pr_debug("** UPDATE NETWORK[%d] ", net.n);
    for(i = 0; i < net.n; ++i){
        layer l = net.layers[i];
        if(l.update){
            pr_debug(", %d", i+1);
...
void backward_network(network *netp)
{
    ...
    int i;
    network orig = net;
    pr_debug("<< BACKWARD NETWORK[%d] ", net.n);
...
float train_network(network *net, data d)
{
    ...
    int batch = net->batch;     ///128
    int n = d.X.rows / batch;   ///rows=128, cols=6912
