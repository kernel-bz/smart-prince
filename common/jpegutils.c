...
{
    /*FIXME: */
    debug_log(LOG_ERR, 0, "%s: Given jpeg buffer was too small", __FUNCTION__);
    ...
    if (*htblptr == NULL)
        *htblptr = jpeg_alloc_huff_table((j_common_ptr) dinfo);
        ...
        if (nsymbols < 1 || nsymbols > 256)
            debug_log(LOG_ERR, 0, "%s: Given jpeg buffer was too small",
                       __FUNCTION__);
                     ...
                     unsigned char *raw1, unsigned char *raw2)
{
    ...
    if (dinfo.output_components != 3) {
        debug_log(LOG_ERR, 0, "%s: Output components of JPEG image = %d, must be 3",
    ...
    if ((hsf[0] != 2 && hsf[0] != 1) || hsf[1] != 1 || hsf[2] != 1 ||
        ...
        debug_log(LOG_ERR, 0, "%s: Unsupported sampling factors, hsf=(%d, %d, %d) "
    ...
    if (hsf[0] == 1) {
        if (height % 8 != 0) {
            debug_log(LOG_ERR, 0, "%s: YUV 4:4:4 sampling, but image height %d "
    ...
    if (dinfo.output_height == height) {
        ...
        debug_log(LOG_ERR, 0, "%s: Read JPEG: requested height = %d, "
    ...
    if (dinfo.output_width > MAX_LUMA_WIDTH) {
        debug_log(LOG_ERR, 0, "%s: Image width of %d exceeds max",
    ...
    for (field = 0; field < numfields; field++) {
        ...
        if (numfields == 2) {
            ...
            case Y4M_ILACE_BOTTOM_FIRST:
                ...
                break;
            default:
                debug_log(LOG_ERR, 0, "%s: Input is interlaced but no interlacing set",
                           __FUNCTION__);
                goto ERR_EXIT;
            }
        } else {
                         ...
                         unsigned char *raw1, unsigned char *raw2)
{
    ...
    if (dinfo.jpeg_color_space != JCS_GRAYSCALE) {
        debug_log(LOG_ERR, 0, "%s: Expected grayscale colorspace for JPEG raw decoding",
                   __FUNCTION__);
        goto ERR_EXIT;
    }
                    ...
                    unsigned char *raw1, unsigned char *raw2)
{
    ...
    if ((width > 4096) || (height > 4096)) {
        debug_log(LOG_ERR, 0, "%s: Image dimensions (%dx%d) exceed lavtools' max "
                   "(4096x4096)", __FUNCTION__, width, height);
        goto ERR_EXIT;
    }

    if ((width % 16) || (height % 16)) {
        debug_log(LOG_ERR, 0, "%s: Image dimensions (%dx%d) not multiples of 16",
    ...
    default:
        numfields = 1;
        if (height > 2048) {
            debug_log(LOG_ERR, 0, "%s: Image height (%d) exceeds lavtools max "
