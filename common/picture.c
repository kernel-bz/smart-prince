...
static void put_ppm_gray_file(FILE *picture, unsigned char *image, int width, int height)
{
    int x, y;
    unsigned char *l = image;
    unsigned char *u = image + width * height;
                       ...
                       unsigned char *image, int quality)
{
    ...
    case VIDEO_PALETTE_GREY:
        return put_jpeg_grey_memory(dest_image, image_size, image,
                                    cnt->imgs.width, cnt->imgs.height, quality);
    default:
        debug_log(LOG_ERR, 0, "Unknow image type %d", cnt->imgs.type);
    }
...
{
    if (cnt->conf.ppm==VIDEO_PALETTE_GREY) {
        put_ppm_gray_file(picture, image, cnt->imgs.width, cnt->imgs.height);
    } else if (cnt->conf.ppm==VIDEO_PALETTE_RGB24) {
...
void put_picture(struct context *cnt, char *file, unsigned char *image, int ftype)
{
    ...
    if (!picture) {
        ...
        if (errno ==  EACCES) {
            ...
            debug_log(LOG_ERR, 1, "Thread is going to finish due to this fatal error");
            cnt->finish = 1;
            cnt->restart = 0;
            return;
        } else {
            /* If target dir is temporarily unavailable we may survive */
            debug_log(LOG_ERR, 1, "Can't write picture to file %s", file);
            return;
        }
    }
...
unsigned char *get_pgm(FILE *picture, int width, int height)
{
    ...
    if (!fgets(line, 255, picture)) {
        debug_log(LOG_ERR, 1, "Could not read from ppm file");
        return NULL;
    }

    if (strncmp(line, "P5", 2)) {
        debug_log(LOG_ERR, 1, "This is not a ppm file, starts with '%s'", line);
        return NULL;
    }
    ...
    if (sscanf(line, "%d %d", &x, &y) != 2) {
        debug_log(LOG_ERR, 1, "Failed reading size in pgm file");
        return NULL;
    }

    if (x != width || y != height) {
        debug_log(LOG_ERR, 1, "Wrong image size %dx%d should be %dx%d", x, y, width, height);
        return NULL;
    }
    ...
    if (sscanf(line, "%d", &maxval) != 1) {
        debug_log(LOG_ERR, 1, "Failed reading maximum value in pgm file");
        return NULL;
    }
    ...
    for (y = 0; y < height; y++) {
        if ((int)fread(&image[y * width], 1, width, picture) != width)
            debug_log(LOG_ERR, 1, "Failed reading image data from pgm file");
...
void put_fixed_mask(struct context *cnt, const char *file)
{
    ...
    if (!picture) {
        ...
        if (errno ==  EACCES) {
            ...
            /* If target dir is temporarily unavailable we may survive */
            debug_log(LOG_ERR, 1, "can't write mask file %s", file);
        }
        return;
    }
    ...
    if ((int)fwrite(cnt->imgs.out, cnt->conf.width, cnt->conf.height, picture) != cnt->conf.height) {
        debug_log(LOG_ERR, 1, "Failed writing default mask as pgm file");
        return;
    }

    fclose(picture);

    debug_log(LOG_ERR, 0, "Creating empty mask %s",cnt->conf.mask_file);
    debug_log(LOG_ERR, 0, "Please edit this file and re-run motion to enable mask feature");
}
